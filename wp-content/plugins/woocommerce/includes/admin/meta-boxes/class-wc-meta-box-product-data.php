...
class WC_Meta_Box_Product_Data {
	...
	public static function output( $post ) {
		...
		$product_type_options = apply_filters( 'product_type_options', array(
			...
			'recommended' => array(
				'id'            => '_recommended',
				'wrapper_class' => 'show_if_simple',
				'label'         => __( 'Recommended Config', 'woocommerce' ),
				'description'   => __( 'This is a Recommended Configuration.', 'woocommerce' ),
				'default'       => 'no'
			)
		) );
				...
				<div class="toolbar toolbar-top">
					<select id="field_to_edit" class="variation_actions">
						...
						<optgroup label="<?php esc_attr_e( 'Status', 'woocommerce' ); ?>">
							...
							<option value="toggle_recommended><?php _e( 'Toggle &quot;Recommended Config&quot;', 'woocommerce' ); ?></option>
						</optgroup>
	...
	public static function save( $post_id, $post ) {
		...
		$is_recommended  = isset( $_POST['_recommended'] ) ? 'yes' : 'no';

		// Product type + Downloadable/Virtual
		wp_set_object_terms( $post_id, $product_type, 'product_type' );
		update_post_meta( $post_id, '_downloadable', $is_downloadable );
		update_post_meta( $post_id, '_virtual', $is_virtual );
		update_post_meta( $post_id, '_recommended', $is_recommended );
	...
	public static function save_variations( $post_id, $post ) {
		...
		if ( isset( $_POST['variable_sku'] ) ) {
			...
			$variable_is_recommended        = isset( $_POST['variable_is_recommended'] ) ? $_POST['variable_is_recommended'] : array();
			...
			for ( $i = 0; $i <= $max_loop; $i ++ ) {
				...
				$is_recommended  = isset( $variable_is_recommended[ $i ] ) ? 'yes' : 'no';
